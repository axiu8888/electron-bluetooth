...
class BinaryHelper {
     ...
     */
    numberToBytes(num, bit, bigEndian = true) {
     ...
     * @return 返回一个整数
     */
    bytesToNumber(bytes, bigEndian = true, signed = false) {
        // 大端存储：高位在前，低位在后
        // 小端存储：低位在前，高位在后
        let value = 0;
     ...
     * @return 返回一个16进制字符串
     */
    numberToHex(value, bitSize, bigEndian = true, upperCase = true) {
     ...
     * @return 返回一个整数
     */
    hexToNumber(hex, bigEndian = true, signed = false) {
