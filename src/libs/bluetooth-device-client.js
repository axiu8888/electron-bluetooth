...
function startScan(options, callback) {
    navigator.bluetooth.requestDevice(options)
        .then(device => {
            let go_on = callback(device, null);
            if (go_on) {
                // 继续扫描
 ...
 * 蓝牙设备连接客户端
 */
class BluetoothDeviceClient extends BinaryHelper {
     ...
     */
    getServiceCharacteristic(serviceUUID, characteristicUUID) {
     ...
     */
    addCharacteristicChanged(characteristics, listener) {
